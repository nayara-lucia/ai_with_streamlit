...
with st.sidebar:
    ...
    num_images_per_prompt = st.slider("Number of images", min_value=1,max_value=5, 
                                      value=1)
    num_inference_steps = st.number_input("Number of inference steps",
                                          min_value=1,max_value=100, value=50)
    height = st.selectbox("Image height",[256,512,768,1024], index=1)
    width = st.selectbox("Image width",[256,512,768,1024], index=1)
    seed = st.number_input("Seed", min_value=0, max_value=99999, value=42)
    guidance_scale = st.number_input("Guidance scale", min_value=1.0, max_value=20.0,
                                     value=7.5)
